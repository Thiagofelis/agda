...
reconstructParameters' act a v = do
  ...
  where
    ...
    loop' ty fTe fTy (Apply u:es) = do
      reportSDoc "tc.reconstruct" 20 $ "The type before app is:" <+> pretty ty
      reportSDoc "tc.reconstruct" 20 $ "The term before app is:" <+> prettyTCM (fTe [])
      uu <- dropParameters u
      reportSDoc "tc.reconstruct" 20 $ "The app is:" <+> pretty uu
      ty' <- piApplyM ty uu
      reportSDoc "tc.reconstruct" 20 $ "The type after app is:" <+> pretty ty'
      loop' ty' (fTe . (Apply u :)) (fTy . (Apply uu :)) es
    loop' ty fTe fTy (Proj o p:es) = do
      reportSDoc "tc.reconstruct" 20 $ "The type is:" <+> pretty ty
      reportSDoc "tc.reconstruct" 20 $ "The term is:" <+> pretty (fTe [])
      reportSDoc "tc.reconstruct" 20 $ "The proj is:" <+> prettyTCM p
      ty' <- reduce ty
      case unEl ty' of
        Def r pars -> do
