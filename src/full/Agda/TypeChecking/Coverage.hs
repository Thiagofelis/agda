...
cover f cs sc@(SClause tel ps _ _ target) = updateRelevance $ do
  ...
  where
    ...
    applyCl SClause{scTel = tel, scPats=pats, scCheckpoints = cps} cl mps = addContext tel $ do
        ...
        reportSDoc "tc.cover.applyCl" 40 $ "old body   =" <+> pretty (clauseBody cl)
        reportSDoc "tc.cover.applyCl" 40 $ "old type   =" <+> pretty (clauseType cl)
        reportSDoc "tc.cover.applyCl" 40 $ "subst body =" <+> pretty (applyPatSubst s $ clauseBody cl)
        let newPats = applySubst s (namedClausePats cl) ++ missingPats
            newBody = (`apply` missingArgs) $ applyPatSubst s $ clauseBody cl
        newType <- case clauseType cl of
                     Just (Arg ai t) -> Just . Arg ai <$> piApplyM (applyPatSubst s t) missingArgs
                     Nothing         -> return Nothing

        reportSDoc "tc.cover.applyCl" 40 $ "new ps   =" <+> pretty newPats
        reportSDoc "tc.cover.applyCl" 40 $ "new body =" <+> pretty newBody
        reportSDoc "tc.cover.applyCl" 40 $ "new type =" <+> pretty newType
        let finalCl =
              cl { clauseTel       = tel
                 , namedClausePats = newPats
                 , clauseBody      = newBody
                 , clauseType      = newType
                 }
        whenM (optDoubleCheck <$> pragmaOptions) $ case (newBody, newType) of
          (Just v, Just vTy)  -> do
            reportSDoc "tc.lhs.top" 30 $ vcat
              [ "double checking rhs"
              , nest 2 (prettyTCM v <+> " : " <+> prettyTCM (unArg vTy))
              ]
            noConstraints $ dontAssignMetas $ checkInternal v $ unArg vTy
          _ -> return ()
        return $ finalCl

      where
        missingPats = fromSplitPatterns $ drop (size (namedClausePats cl)) pats
        missingArgs = fromMaybe __IMPOSSIBLE__ $ allApplyElims $ patternsToElims missingPats
        (vs,qs) = unzip mps
...
computeNeighbourhood delta1 n delta2 d pars ixs hix tel ps cps c = do
  ...
  case r of
    ...
    Unifies (delta1',rho0,_) -> do
      debugSubst "rho0" rho0
      isRecord <- liftTCM $ defIsRecord. theDef <$> getConstInfo d
      ...
      -- Andreas, 2017-09-03, issue #2729: remember that pattern was generated by case split.
      let
          pats = applySubst rho0' $ map (mapArgInfo hiddenInserted) $ telePatterns' (tele2NamedArgs gamma0) gamma boundary
      typeFullyApplied <- piApplyM (piApply ctype pars) (map (patternToTerm . namedArg) (fromSplitPatterns pats))
      let cpi  =
            if isRecord
            then noConPatternInfo{ conPRecord = Just PatOSplit, conPType = Just $ defaultArg typeFullyApplied}
            else noConPatternInfo
          conp = ConP con cpi $ pats
          -- Andreas, 2016-09-08, issue #2166: use gamma0 for correct argument names
